...
function oneHot80(draw){
  const v=Array(RANGE).fill(0);
  for(const n of draw.balls)v[n-1]=1;
  return v;
}
...
function topN(probs,n=20){
  return Array.from({length:RANGE},(_,i)=>i+1)
    .sort((a,b)=>(probs[b-1]-probs[a-1])||(a-b))
    .slice(0,n);
}
...
if(all.length<1006){
  throw new Error(`XRAY TRAIN: мало истории (${all.length}), нужно минимум 1006`);
}
...
if(xs.length<1000){
  throw new Error(`XRAY TRAIN: мало окон 5→1 (${xs.length})`);
}

const x=tf.tensor3d(xs,[xs.length,WINDOW,RANGE]);
const y=tf.tensor2d(ys,[ys.length,RANGE]);

const split=Math.floor(xs.length*0.80);
const xTrain=x.slice([0,0,0],[split,-1,-1]);
const yTrain=y.slice([0,0],[split,-1]);

const model=buildModel();
model.summary();

/*
 * TensorFlow.js 4.22.0 does NOT implement restoreBestWeights=true
 * for tf.callbacks.earlyStopping().  Using that option makes the
 * GitHub Action fail before epoch 1.  We keep early stopping, but
 * without restoreBestWeights.
 */
...
const last5=all.slice(-WINDOW);
const px=tf.tensor3d([last5.map(oneHot80)],[1,WINDOW,RANGE]);
const py=model.predict(px);
const p=Array.from(await py.data()).slice(0,RANGE);
const preview=topN(p,20);

const meta={
  version:'XRAY-AI-2.0.1',
  architecture:'Attention LSTM 5→1 adapted from kyr0/lotto-ai',
  window:WINDOW,
  range:RANGE,
  numbersPerDraw:20,
  ...
  loss:'binaryCrossentropy',
  epochsMax:EPOCHS,
  earlyStopping:{monitor:'val_loss',patience:10,restoreBestWeights:false},
  batchSize:BATCH_SIZE,
  historyDrawsUsed:trainDraws.length,
  trainingWindows:xs.length,
  latestDraw:all.at(-1).draw,
  latestDate:all.at(-1).date,
  latestTime:all.at(-1).time,
  trainedAt:new Date().toISOString(),
  previewTop20:preview
};
