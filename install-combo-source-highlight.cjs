...
// Подсветка исходных чисел в предыдущем тираже.
rep(
...
// Функции: какие числа надо подсветить в конкретной предыдущей карточке.
rep(
...
// В карточке вместо простого текста 20 чисел — отдельные span, чтобы подсвечивать нужные.
rep(
  `<div class="recentDrawTop"><span>№${d.draw}</span><span>${d.date} ${d.time}</span></div><div class="recentDrawNums">${d.balls.map(fmt).join(' ')}</div>${comboDrawDetailsHTML(d)}</button>`,
  `<div class="recentDrawTop"><span>№${d.draw}</span><span>${d.date} ${d.time}</span></div><div class="recentDrawNums">${comboRecentNumbersHTML(d)}</div>${comboDrawDetailsHTML(d)}</button>`,
  'recent card numbers'
);

fs.writeFileSync('index.html', h, 'utf8');

// Новый cache, чтобы телефон точно получил v4.1.14.
