...
function parseSource(text) {
  ...
  return htmlRows.length ? htmlRows : parseReaderText(text);
}
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      const response = await fetch(url, {
        headers: {
          'user-agent': 'Mozilla/5.0 GitHub-Actions Positron-Combo-v1/1.0.1',
...
async function main() {
  ...
  if (!changed && fs.existsSync(STATUS_FILE)) {
    console.log(`ПОЗИТРОН КОМБО v1: новых тиражей нет, последний №${latest.draw}`);
    return;
  }
  ...
  console.log(`ПОЗИТРОН КОМБО v1: база ${draws.length}, последний №${latest.draw}, источник ${source}`);
}
