...
function fail(msg) {
  throw new Error('COMBO TRANSITIONS INSTALL FAIL: ' + msg);
}

let js = fs.readFileSync(SEARCH, 'utf8');

if (!js.includes("function detailRow(d,nums)")) {
  fail('не найдена исходная detailRow(d,nums)');
}
if (!js.includes("⬆️ Возрастание</button>")) {
  fail('не найдена кнопка «Возрастание»');
}
if (!js.includes("visible.map(d=>detailRow(d,row.nums))")) {
  fail('не найден вывод detailRow');
}
...
if (!js.includes('csTransitionsBtn')) {
  ...
  if (!js.includes(oldRow)) {
    fail('не удалось точно заменить detailRow');
  }
  ...
  const oldButtons =
    '<div class="csDetailTools"><div class="historyTools"><button type="button" class="active">⬆️ Возрастание</button></div><label class="csDrawCount">';

  const newButtons =
    `<div class="csDetailTools"><div class="historyTools"><button type="button" class="active">⬆️ Возрастание</button><button id="csTransitionsBtn" type="button" class="\${showTransitions?'active':''}" aria-pressed="\${showTransitions?'true':'false'}">🔸 Переходы</button></div><label class="csDrawCount">`;

  if (!js.includes(oldButtons)) {
    fail('не совпала строка кнопки «Возрастание»');
  }
  ...
  if (!js.includes('transitionSetForDraw')) {
    fail('не установлен расчёт переходов');
  }
  if (!js.includes('detailRow(d,row.nums,showTransitions)')) {
    fail('не передан флаг showTransitions');
  }
  if (!js.includes('⬆️ Возрастание</button><button id="csTransitionsBtn"')) {
    fail('кнопки не стоят в одной строке в нужном порядке');
  }
...
fs.writeFileSync(
  VERSION,
  JSON.stringify({version:'4.1.24'}, null, 2) + '\n',
  'utf8'
);
